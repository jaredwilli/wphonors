<?php
/*
Plugin Name: I Like This Reloaded
Plugin URI: http://new2wp.com/i-like-this-reloaded/
Description: This plugin allows your visitors to simply vote for your posts instead of comment it.
Version: 2.5
...
function ILikeThisAdminMenu() {
	add_options_page('I Like This', 'I Like This', 'manage_options', 'ILikeThisAdminMenu', 'ILikeThisAdminContent');
}
...
function add_widget_most_liked_posts() {
	...
	wp_register_widget_control('Most liked posts', 'options_widget_most_liked_posts');
} 
...
function getILikeThis($arg) {
    ...
    if (!isset($_COOKIE['liked-' . $post_ID ]) && $voteStatusByIp == 0) {
    	...
    	if (get_option('ilt_textOrImage') == 'image') {
    		...
    		$counter = '<a onclick="likeThis(' . $post_ID . ');">' . get_option('ilt_text') . '</a>'; // $liked
    ...
    $iLikeThis = '<div id="iLikeThis-'.$post_ID.'" class="iLikeThis">';
		if ($voteStatusByIp == '1') {
			$iLikeThis .= '<span class="inactive">'.$counter.'</span>';
