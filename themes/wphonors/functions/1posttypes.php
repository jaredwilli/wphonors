...
class TypeFinalists {
    ...
    // Store the data
    public $meta_fields = array( 'title', 'excerpt', 'ptype', 'posturl', 'post_category', 'twitname' );
	...
	$ptype = get_post_meta($post->ID, 'ptype', true );
        switch ($column) {
            case 'title' : the_title();
                break;
            case 'posturl' : $b = $this->mshot(); echo '<a href="'.$b[0].'" target="_blank">'.$b[0].'</a>';
                break;
            ...
            case 'post_author' : the_author();
                break;
            case 'finalistimg' : $b = $this->mshot(100); echo '<img src="'.$b[1].'" width="100" />';
                break;
        }
    }
	...
	*/
    ...
    public function post_url() {
        global $post, $posturl;
        $posturl = get_post_custom($post->ID, 'posturl', true );
        
        $myurl = trailingslashit( get_post_meta( $post->ID, 'posturl', true ) );
        if ( $myurl != '' ) {
            if ( preg_match( "/http(s?):\/\//", $myurl )) {
                $posturl = get_post_meta( $post->ID, 'posturl', true );
                $mshoturl = 'http://s.WordPress.com/mshots/v1/' . urlencode( $myurl );
            } else {
                $posturl = 'http://'.get_post_meta( $post->ID, 'posturl', true );
                $mshoturl = 'http://s.WordPress.com/mshots/v1/' . urlencode('http://'.$myurl);
            }
            $imgsrc  = '<img src="' . $mshoturl . '?w=250" width="250" />';
        } ?>
	
        <p><label for="posturl">Enter the Submission Url below:<br />
        <input id="posturl" size="37" name="posturl" value="<?php echo $posturl; ?>" /></label></p>
		<p><?php echo '<a href="' . $posturl . '">' . $imgsrc . '</a>'; ?></p>
    ...
    public function mshot($mshotsize) {
        global $post;
        $imgWidth = $mshotsize;
        $myurl = get_post_meta($post->ID, 'posturl', true);
        $mshoturl = '';
        if ( $myurl != '' ) {
            if ( preg_match( "/http(s?):\/\//", $myurl )) {
                $posturl = get_post_meta( $post->ID, 'posturl', true );
